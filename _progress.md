...
## 2026-08-18 Progress (grid menu — functional on mobile)
- Measured the rendered site across real phone viewports (iPhone SE 320px, iPhone 13 390px, Pixel 7 412px), day + night, using tooling/. The 2x2 layout itself is already sound on mobile: four cells as direct children of .grid-menu, tap-target boxes 135-181px (far above the 44px WCAG minimum), all four labels visible, images un-dimmed -- so it is NOT a layout-shape problem, and the earlier "stack vs enlarge" question is resolved in favour of keeping the 2x2 grid.
- Found and fixed the one genuine non-aesthetic mobile defect: the always-visible mobile caption was a fixed font-size of 1.4em (~24px). The one-word label "Accommodations" is longer than a 320px phone cell, and because each .grid-menu a has overflow:hidden, the word was being CUT OFF -- by up to 65px on iPhone SE (30px on iPhone 13, 19px on Pixel 7). The prior tooling missed it: its overflow check tested the label's own overflow (default: visible) instead of the label overflowing its parent cell box.
- Fix (css/gridmenu.css, inside the @media (max-width:600px) block): replaced font-size: 1.4em with font-size: clamp(15px, 4.5vw, 20px) so the caption scales with the viewport down to a legible 15px floor, plus overflow-wrap: anywhere; word-break: break-word as a last-resort safety net so a pathological word wraps to a second line instead of clipping. Desktop hover behaviour (labels hidden + image dim until hover) is untouched -- the change lives only in the mobile media query.
- Added decoding="async" to the four grid <img> tags in index.qmd (no image generated or altered): the standard hint that keeps image decoding off the main render path. No loading="lazy" on purpose -- these are the page's above-the-fold hero images.
